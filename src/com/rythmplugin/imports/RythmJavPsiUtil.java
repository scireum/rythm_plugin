 ...
 */
public class RythmJavPsiUtil {

    private static final Logger LOG = Logger.getInstance("#" + RythmJavPsiUtil.class.getName());
    ...
    public static PsiClassType getPropertyClassType(PsiElement member) {
        return getPropertyClassType(member, RythmJavCommonNames.JAVAFX_BEANS_PROPERTY_OBJECT_PROPERTY);
    }
    ...
    public static PsiMethod findObservablePropertyGetter(@NotNull PsiClass psiClass, @Nullable String propertyName) {
        if (StringUtil.isEmpty(propertyName)) return null;
        final PsiMethod[] getters = psiClass.findMethodsByName(propertyName + RythmJavCommonNames.PROPERTY_METHOD_SUFFIX, true);
        for (PsiMethod getter : getters) {
            if (getter.hasModifierProperty(PsiModifier.PUBLIC) &&
                    !getter.hasModifierProperty(PsiModifier.STATIC) &&
                    getter.getParameterList().getParametersCount() == 0 &&
                    InheritanceUtil.isInheritor(getter.getReturnType(),RythmJavCommonNames.JAVAFX_BEANS_VALUE_OBSERVABLE_VALUE)) {
                return getter;
            }
        }
        return null;
    ...
    public static PsiClass getControllerClass(final PsiFile containingFile) {
        if (containingFile instanceof XmlFile) {
            ...
            if (rootTag != null) {
                XmlAttribute attribute = rootTag.getAttribute(RythmJavConstants.FX_CONTROLLER);
                if (attribute != null) {
            ...
            if (rootTag != null && RythmJavConstants.FX_ROOT.equals(rootTag.getName())) {
                final XmlAttribute rootTypeAttr = rootTag.getAttribute(RythmJavConstants.TYPE);
    ...
    public static boolean isVisibleInFxml(@NotNull PsiMember psiMember) {
        return psiMember.hasModifierProperty(PsiModifier.PUBLIC) ||
                AnnotationUtil.isAnnotated(psiMember, RythmJavCommonNames.JAVAFX_FXML_ANNOTATION, false);
    }
    ...
    public static PsiMethod findValueOfMethod(@NotNull final PsiClass psiClass) {
        return CachedValuesManager.getCachedValue(psiClass, () -> {
            final PsiMethod[] methods = psiClass.findMethodsByName(RythmJavCommonNames.VALUE_OF, true);
    ...
    public static boolean isReadOnly(String attributeName, XmlTag tag) {
        ...
        if (descriptor instanceof RythmJavClassTagDescriptorBase) {
            return ((RythmJavClassTagDescriptorBase)descriptor).isReadOnlyAttribute(attributeName);
        }
        return false;
    }
    ...
    public static PsiType getWritablePropertyType(@Nullable final PsiType type, @NotNull final Project project) {
        ...
        if (psiClass != null) {
            final PsiClass propertyClass = JavaPsiFacade.getInstance(project).findClass(RythmJavCommonNames.JAVAFX_BEANS_PROPERTY,
    ...
    private static PsiType getDefaultPropertyExpectedType(@Nullable PsiClass aClass) {
        ...
        return CachedValuesManager.getCachedValue(aClass, () -> {
            final PsiAnnotation annotation =
                    AnnotationUtil.findAnnotationInHierarchy(aClass, Collections.singleton(RythmJavCommonNames.JAVAFX_BEANS_DEFAULT_PROPERTY));
            if (annotation != null) {
    ...
    public static String getDefaultPropertyName(@Nullable PsiClass aClass) {
        ...
        final PsiAnnotation annotation = AnnotationUtil.findAnnotationInHierarchy(aClass,
                Collections.singleton(
                        RythmJavCommonNames.JAVAFX_BEANS_DEFAULT_PROPERTY));
        if (annotation != null) {
            final PsiAnnotationMemberValue memberValue = annotation.findAttributeValue(null);
            if (memberValue != null) {
        ...
        return CachedValuesManager.getCachedValue(psiClass, () -> {
            for (PsiMethod constructor : psiClass.getConstructors()) {
                ...
                for (PsiParameter parameter : parameters) {
                    if (!AnnotationUtil.isAnnotated(parameter, RythmJavCommonNames.JAVAFX_BEANS_NAMED_ARG, false)) {
                        annotated = false;
                        break;
                    }
                }
    ...
    public static boolean hasBuilder(@NotNull final PsiClass psiClass) {
        return CachedValuesManager.getCachedValue(psiClass, () -> {
            ...
            final PsiClass builderClass = JavaPsiFacade.getInstance(project).findClass(RythmJavCommonNames.JAVAFX_FXML_BUILDER,
                    GlobalSearchScope.allScope(project));
    ...
    public static boolean isClassAcceptable(@Nullable XmlTag targetTag, @Nullable final PsiClass fromClass,
        ...
        if (tagDescriptor instanceof RythmJavPropertyTagDescriptor) {
            final PsiClass containingClass = ((RythmJavPropertyTagDescriptor)tagDescriptor).getPsiClass();
        ...
        else if (tagDescriptor instanceof RythmJavClassTagDescriptorBase) {
            ...
            if (tagDeclaration instanceof PsiClass) {
                ...
                final XmlAttribute factoryAttr = targetTag.getAttribute(RythmJavConstants.FX_FACTORY);
                if (factoryAttr != null) {
    ...
    private static boolean canCoerce(@Nullable PsiType targetType, @NotNull PsiClass fromClass, @NotNull PsiElement context,
        ...
        if (collectionItemType == null && InheritanceUtil.isInheritor(targetType, RythmJavCommonNames.JAVAFX_BEANS_PROPERTY)) {
    ...
    public static boolean isOutOfHierarchy(final XmlAttributeValue element) {
        ...
        while (tag != null) {
            if (RythmJavConstants.FX_DEFINE.equals(tag.getName())) {
                return true;
            }
    ...
    public static PsiType getWrappedPropertyType(final PsiField field, final Project project, final Map<String, PsiType> typeMap) {
        return CachedValuesManager.getCachedValue(field, () -> {
            ...
            if (substitute == null) {
                if (!InheritanceUtil.isInheritor(fieldType, RythmJavCommonNames.JAVAFX_BEANS_VALUE_OBSERVABLE_VALUE)) {
                    final PsiType propertyType = eraseFreeTypeParameters(fieldType, field);
                    return CachedValueProvider.Result.create(propertyType, PsiModificationTracker.JAVA_STRUCTURE_MODIFICATION_COUNT);
                }
                final PsiClass aClass = JavaPsiFacade.getInstance(project)
                        .findClass(RythmJavCommonNames.JAVAFX_BEANS_VALUE_OBSERVABLE_VALUE, GlobalSearchScope.allScope(project));
                LOG.assertTrue(aClass != null);
                final PsiSubstitutor substitutor =
                        TypeConversionUtil.getSuperClassSubstitutor(aClass, fieldClass, resolveResult.getSubstitutor());
                final PsiMethod[] values = aClass.findMethodsByName(RythmJavCommonNames.GET_VALUE, false);
    ...
    public static PsiType getWritablePropertyType(@Nullable PsiClass containingClass, @Nullable PsiElement declaration) {
        if (declaration instanceof PsiField) {
            return getWrappedPropertyType((PsiField)declaration, declaration.getProject(), RythmJavCommonNames.ourWritableMap);
        }
    ...
    public static PsiType getReadablePropertyType(@Nullable PsiElement declaration) {
        if (declaration instanceof PsiField) {
            return getWrappedPropertyType((PsiField)declaration, declaration.getProject(), RythmJavCommonNames.ourReadOnlyMap);
        }
        if (declaration instanceof PsiMethod) {
    ...
    public static Map<String, XmlAttributeValue> collectFileIds(@Nullable final XmlTag currentTag) {
        ...
        final XmlAttribute currentIdAttribute = currentTag.getAttribute(RythmJavConstants.FX_ID);
    ...
    private static Map<String, XmlAttributeValue> prepareFileIds(XmlTag rootTag) {
        ...
        for (XmlTag tag : SyntaxTraverser.psiTraverser().withRoot(rootTag).filter(XmlTag.class)) {
            final XmlAttribute idAttribute = tag.getAttribute(RythmJavConstants.FX_ID);
            if (idAttribute != null) {
                final String idValue = idAttribute.getValue();
                if (idValue != null) fileIds.put(idValue, idAttribute.getValueElement());
            }
        }
        final XmlAttribute controllerAttribute = rootTag.getAttribute(RythmJavConstants.FX_CONTROLLER);
        if (controllerAttribute != null) {
            fileIds.put(RythmJavConstants.CONTROLLER, controllerAttribute.getValueElement());
        }
        return fileIds;
    }
    ...
    public static PsiClass getTagClassById(@Nullable XmlAttributeValue xmlAttributeValue, @Nullable String id, @NotNull PsiElement context) {
        return RythmJavConstants.CONTROLLER.equals(id) ? getControllerClass(context.getContainingFile()) : getTagClass(xmlAttributeValue);
    }
   ...
   @Nullable
    private static String getPropertyNameFromNamedArgAnnotation(@NotNull PsiParameter parameter) {
        final PsiAnnotation annotation = AnnotationUtil.findAnnotation(parameter, RythmJavCommonNames.JAVAFX_BEANS_NAMED_ARG);
        if (annotation != null) {
            final PsiAnnotationMemberValue psiValue = annotation.findAttributeValue(RythmJavCommonNames.VALUE);
    ...
    private static boolean isWritablePropertyType(@NotNull PsiClass psiClass, @NotNull PsiType fieldType) {
        return isObservableCollection(PsiUtil.resolveClassInType(fieldType)) &&
                ...
                InheritanceUtil.isInheritor(fieldType, RythmJavCommonNames.JAVAFX_COLLECTIONS_OBSERVABLE_MAP);
    }

    public static boolean isObservableCollection(@Nullable PsiClass psiClass) {
        return psiClass != null &&
                (InheritanceUtil.isInheritor(psiClass, RythmJavCommonNames.JAVAFX_COLLECTIONS_OBSERVABLE_LIST) ||
                        InheritanceUtil.isInheritor(psiClass, RythmJavCommonNames.JAVAFX_COLLECTIONS_OBSERVABLE_SET) ||
                        InheritanceUtil.isInheritor(psiClass, RythmJavCommonNames.JAVAFX_COLLECTIONS_OBSERVABLE_ARRAY));
    }
    ...
    private static PsiSubstitutor getTagClassSubstitutor(@NotNull XmlAttribute xmlAttribute, @NotNull PsiClass controllerClass) {
        ...
        if (tagClass != null) {
            final String tagFieldName = xmlTag.getAttributeValue(RythmJavConstants.FX_ID);
    ...
    private static PsiType getEventHandlerPropertyType(@NotNull PsiClass tagClass, @NotNull String eventName) {
        ...
        for (PsiMethod handlerSetter : handlerSetterCandidates) {
            if (!handlerSetter.hasModifierProperty(PsiModifier.STATIC) &&
                ...
                if (InheritanceUtil.isInheritor(propertyType, RythmJavCommonNames.JAVAFX_EVENT_EVENT_HANDLER)) {
                    return propertyType;
                }
            }
        }
    ...
    private static PsiClassType substituteEventType(@Nullable PsiType eventHandlerType, @NotNull Project project) {
        ...
        final PsiClass eventHandlerInterface =
                JavaPsiFacade.getInstance(project).findClass(RythmJavCommonNames.JAVAFX_EVENT_EVENT_HANDLER, GlobalSearchScope.allScope(project));
    ...
    public static Pair<PsiClass, Boolean> getTagValueClass(@NotNull XmlTag xmlTag, @Nullable PsiClass tagClass) {
        if (tagClass != null) {
            final XmlAttribute constAttr = xmlTag.getAttribute(RythmJavConstants.FX_CONSTANT);
            if (constAttr != null) {
                ...
                final XmlAttribute factoryAttr = xmlTag.getAttribute(RythmJavConstants.FX_FACTORY);
                if (factoryAttr != null) {
    ...
    private static class JavaFxControllerCachedValueProvider implements CachedValueProvider<PsiClass> {
        ...
        public Result<PsiClass> compute() {
            ...
            final PsiClass fxmlLoader =
                    JavaPsiFacade.getInstance(myProject).findClass(RythmJavCommonNames.JAVAFX_FXML_FXMLLOADER, GlobalSearchScope.allScope(myProject));
            if (fxmlLoader != null) {
                    ...
                    ReferencesSearch.search(myContainingFile, globalSearchScope).forEach(reference -> {
                        ...
                        if (element instanceof PsiLiteralExpression) {
                            ...
                            if (expression != null) {
                                ...
                                if (type != null && type.equalsToText(RythmJavCommonNames.JAVAFX_FXML_FXMLLOADER)) {
